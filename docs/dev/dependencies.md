...
### `github.com/hashicorp/go-plugin` — out-of-process backends

M9's loading mechanism: the handshake, the subprocess lifecycle, the unix socket,
and the broker that lets a plugin serve something back to the host. It brings
`hashicorp/yamux` (stream multiplexing), `hashicorp/go-hclog` (its logger, which
drivel silences in favour of the per-mount logger), `oklog/run` and the
`fatih/color` / `mattn/go-*` chain hclog uses for terminal output.

Go's own `plugin` package was the obvious alternative and is not usable: Linux
only, requires the host and the plugin to have been built by the same toolchain
from the same dependency versions, and nothing can ever be unloaded. Writing the
subprocess protocol by hand was the other option; go-plugin is that protocol,
already debugged, including the parts that are easy to get subtly wrong (handshake
timeouts, reaping, mTLS on the control connection, the reverse channel).

A WASM host would have given a real sandbox, which this does not. It would also
have given a storage backend no sockets, no filesystem and no OAuth — so the
sandbox would have had to be opened up for exactly the things that make it worth
having.

### `google.golang.org/grpc` and `google.golang.org/protobuf` — the plugin protocol

What the host and a backend speak (M9). Both were already in the tree as indirect
dependencies of the Drive client, so this promoted them rather than adding them.
gRPC carries streaming, which is what `Put`, `Get` and `GetRange` need for a file
of any size, and status details, which is how the three sentinel errors survive the
process boundary.

The generated Go is committed under `plugin/internal/pb` and regenerated by
`make proto`. The toolchain — `buf`, `protoc-gen-go`, `protoc-gen-go-grpc` — is
pure Go and pinned in the Makefile; **there is no `protoc`**, and none should be
added. `make proto-check` runs in `make check` and fails if the committed output
...
| Not used | Why |
| --- | --- |
| **A logging library** | The standard `log` package, one logger per mount. Structured logging would be the first dependency whose value is a matter of taste. (`go-hclog` arrives with go-plugin and is used by neither half of drivel.) |
...
| **A cloud SDK beyond the ones in use** | A new provider brings its own client, inside its own package, below the seam — as `gdrive` and `sftp` each do. Since M9 it is linked into that backend's own executable and into nothing else. |
| **`protoc`** | The plugin protocol is generated by `buf`, which is pure Go and installs with `go install`. A C++ compiler in the build for one codegen step would undo what the cgo row above is protecting. |
| **A logging bridge for hclog** | go-plugin's logger is silenced and the plugin's stderr is forwarded verbatim to the per-mount logger instead. Two log formats in one stream is worse than one. |
