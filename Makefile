...
# compiler as well as the driver. protoc-gen-go's version is pinned to the
# google.golang.org/protobuf in go.mod, because the generated code and the
# runtime that reads it are one pair.
BUF_VERSION                ?= v1.58.0
...
# Where the provider plugins go. It is on drivel's default search path, and it is
# under lib rather than bin because these are not commands a user runs.
PLUGINDIR ?= $(PREFIX)/lib/drivel/plugins
...
# bin/, so these two keep their plain names. The stamped copies beside them are
# what makes a version bump reinstall rather than silently reuse.
PROTOC_GEN_GO      := $(BIN)/protoc-gen-go-$(PROTOC_GEN_GO_VERSION)-$(GOVERSION)
PROTOC_GEN_GO_GRPC := $(BIN)/protoc-gen-go-grpc-$(PROTOC_GEN_GO_GRPC_VERSION)-$(GOVERSION)
...
# The shell completions are not generated here and are not committed: drivel
# prints its own, from the flag definitions in cmd/drivel. `drivel completion
# bash` is the whole mechanism, which is what makes a downloaded single binary
# able to install its own completion with no repository and no Makefile behind
# it. See cmd/drivel/completions.go.
#
# There is no completions-check either. What that target guarded — a flag added
# without a completion hint, or a hint naming a flag that no longer exists — is
# now a unit test (cmd/drivel/completions_test.go), because the failure it
# catches is in the program rather than in a file that could drift from it.

# The plugin protocol's generated Go (M9): generated by a pinned toolchain,
# committed because whoever builds from a tarball has no protobuf toolchain, and
# checked so the committed copy cannot drift from the .proto it came from. The
# completions used to work this way too and no longer do — the binary prints its
# own, which is a route a .proto has no equivalent of.
#> make proto                       # regenerate plugin/internal/pb after editing the .proto
## proto: regenerate the plugin protocol's Go from proto/
.PHONY: proto
proto: $(BUF) $(PROTOC_GEN_GO) $(PROTOC_GEN_GO_GRPC)
	$(BUF) generate

## proto-check: fail if the committed generated protocol code is stale
.PHONY: proto-check
proto-check: $(BUF) $(PROTOC_GEN_GO) $(PROTOC_GEN_GO_GRPC)
	@$(BUF) generate
	@git diff --quiet -- plugin/internal/pb || { \
		echo "plugin/internal/pb is stale; run 'make proto' and commit the result"; \
		git --no-pager diff --stat -- plugin/internal/pb; \
		exit 1; \
	}
...
# "_drivel" from $fpath. They are produced by the binary that was just built,
# which is why nothing is committed and why there is nothing to keep in step.
#
# Note that this runs the built binary, so `make install` after cross-compiling
# needs an emulator or a second native build. That is the trade for a binary that
# can install its own completion anywhere; it is also what every other Go CLI
# does, and it replaces a worse requirement (a Go toolchain at package time).
	install -d $(DESTDIR)$(BASHCOMPDIR) $(DESTDIR)$(ZSHCOMPDIR)
	$(BIN)/drivel completion bash > $(BIN)/drivel.bash
	$(BIN)/drivel completion zsh > $(BIN)/_drivel
	install -m0644 $(BIN)/drivel.bash $(DESTDIR)$(BASHCOMPDIR)/drivel
	install -m0644 $(BIN)/_drivel $(DESTDIR)$(ZSHCOMPDIR)/_drivel

## uninstall: remove what install placed
.PHONY: uninstall
uninstall:
	rm -f $(DESTDIR)$(PREFIX)/bin/drivel $(DESTDIR)$(MANDIR)/man1/drivel.1
	@for p in $(PLUGINS); do rm -f $(DESTDIR)$(PLUGINDIR)/$$p; done
...
# buf invokes these by their plain names, so each rule installs the plain binary
# and copies it to the stamped path the dependency is expressed against.
$(PROTOC_GEN_GO):
	@mkdir -p $(BIN)
	GOBIN=$(BIN) go install google.golang.org/protobuf/cmd/protoc-gen-go@$(PROTOC_GEN_GO_VERSION)
	@cp $(BIN)/protoc-gen-go $@

$(PROTOC_GEN_GO_GRPC):
	@mkdir -p $(BIN)
	GOBIN=$(BIN) go install google.golang.org/grpc/cmd/protoc-gen-go-grpc@$(PROTOC_GEN_GO_GRPC_VERSION)
	@cp $(BIN)/protoc-gen-go-grpc $@
